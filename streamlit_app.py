# --- VERSÃO 8.3 - COM PAINEL DE DEPURAÇÃO ---
...
st.title("🤖 Agente de Viabilidade de Projetos v8.3 (Debug)")
...
with st.sidebar:
    ...
    with st.expander("🐞 Painel de Depuração"):
        st.write("Estado atual da sessão:")
        st.json(st.session_state.to_dict()) # Usando st.json para melhor visualização
...
with col1:
    ...
    if st.button("🚀 Iniciar Pesquisa", disabled=(not all([st.session_state.gemini_key, st.session_state.search_key, st.session_state.search_id]))):
        if cidade_input and pergunta:
            ...
            st.session_state.etapa = 'buscando_leis'
            st.rerun()
        else:
    ...
    try:
        ...
        with st.spinner("Pesquisando documentos..."):
            ...
            if 'items' in resultado:
                ...
                st.session_state.etapa = 'selecionando_leis'
            else:
                st.error("Nenhum documento encontrado na busca."); st.session_state.etapa = 'inicio'
    except Exception as e:
...
if st.session_state.etapa in ['buscando_fomento', 'concluido']:
    if 'analise_final' not in st.session_state:
        with st.spinner("🧠 Consultor Sênior analisando o dossiê..."):
            ...
            prompt_analise = f"""(Seu prompt completo)"""
...
if st.session_state.etapa == 'concluido':
    with col2:
        st.subheader("3. Análise Consolidada")
